...
public class FirstPersonController : MonoBehaviour
{
    ...
    public LayerMask cameraCollisionLayers = ~0; // все слои
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                // Проверяем, есть ли реальный ввод от игрока (нажаты ли кнопки движения)
                bool isMoving = Mathf.Abs(Input.GetAxis("Horizontal")) > 0.05f || Mathf.Abs(Input.GetAxis("Vertical")) > 0.05f;
                ...
                {
                    // Если игрок держит Shift, но не нажимает кнопки движения — отключаем состояние бега
                    isSprinting = false;
                }
    ...
    private void HandleCameraCollision()
    {
        ...
        Vector3 desiredLocalPos = cameraDefaultLocalPos;

        // Получаем желаемую мировую позицию камеры
        Vector3 desiredWorldPos = joint.TransformPoint(desiredLocalPos);
        ...
        int layerMask = cameraCollisionLayers & ~(1 << playerLayer);

        if (Physics.SphereCast(origin, cameraCollisionRadius, direction.normalized, out RaycastHit hit, distance, layerMask))
        {
            float safeDistance = Mathf.Clamp(hit.distance - cameraCollisionRadius, 0f, distance);
            Vector3 safeWorldPos = origin + direction.normalized * safeDistance;
            // Переводим обратно в localPosition
            playerCamera.transform.localPosition = joint.InverseTransformPoint(safeWorldPos);
        }
        else
        {
            playerCamera.transform.localPosition = desiredLocalPos;
        }
    }
}
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        #region Camera Collision Setup

        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Camera Collision Setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.enableCameraCollision = EditorGUILayout.ToggleLeft(new GUIContent("Enable Camera Collision", "Prevents the camera from clipping through objects."), fpc.enableCameraCollision);

        GUI.enabled = fpc.enableCameraCollision;
        fpc.cameraCollisionRadius = EditorGUILayout.Slider(new GUIContent("Collision Radius", "Radius of the sphere used for camera collision detection."), fpc.cameraCollisionRadius, 0.05f, 0.5f);
        fpc.cameraCollisionLayers = EditorGUILayout.MaskField(new GUIContent("Collision Layers", "Layers the camera will collide with."), fpc.cameraCollisionLayers, UnityEditorInternal.InternalEditorUtility.layers);
        GUI.enabled = true;
